...
function tokenizeForSimilarity(value) {
  return normalizeForDuplicate(value).split(" ").filter(Boolean).filter((token) => !["a", "an", "the", "shall", "must", "should", "it"].includes(token)).map(stemLightly);
}
...
var audit_run_schema_default = {
  $schema: "https://json-schema.org/draft/2020-12/schema",
  $id: "https://audit-canvas.dev/schemas/audit-run.schema.json",
  title: "AuditCanvas AuditRun",
  type: "object",
  additionalProperties: false,
  required: [
    "auditRunId",
  ...
  properties: {
    auditRunId: { type: "string", minLength: 1 },
    schemaVersion: { type: "string", const: "0.1.0" },
    baselineCommit: { type: ["string", "null"] },
    targetCommit: { type: "string", minLength: 1 },
    sourceBlockCount: { type: "integer", minimum: 0 },
    coveredBlockCount: { type: "integer", minimum: 0 },
    excludedBlockCount: { type: "integer", minimum: 0 },
    coverage: {
      type: "object",
      additionalProperties: false,
      required: ["percentage", "excludedSourceBlocks"],
      properties: {
    ...
    sourceBlocks: { type: "array", items: { $ref: "#/$defs/sourceBlock" } },
    invalidProviderResults: {
      type: "array",
      items: {
        type: "object",
        additionalProperties: false,
        required: ["provider", "reason", "createdAt"],
        properties: {
  ...
  $defs: {
    artifact: {
      ...
      properties: {
        ...
        artifactType: { enum: ["markdown", "txt", "json", "source"] },
    ...
    sourceBlock: {
      ...
      properties: {
        ...
        artifactId: { type: "string", minLength: 1 },
        blockType: { enum: ["heading", "paragraph", "json", "source", "text"] },
        ...
        previousBlockId: { type: ["string", "null"] },
        nextBlockId: { type: ["string", "null"] }
      }
    },
    evidence: {
      type: "object",
    ...
    finding: {
      ...
      properties: {
        ...
        relatedBlockIds: { type: "array", items: { type: "string" } },
        ...
        generatedBy: { enum: ["local-rule", "provider", "human"] },
...
function parseArtifact(input) {
  ...
  const sourceBlocks = rawBlocks.map((block) => {
...
function parseBlocks(artifactType, content) {
...
function parsePlainBlocks(content, blockType) {
  const lines = content.replace(/\r\n/g, "\n").split("\n");
  const blocks = [];
  let current = [];
  let startLine = 1;
...
function flattenJson(value, path, blocks) {
...
// packages/core/dist/coverage.js
function computeCoverage(sourceBlocks, findings, exclusions = []) {
...
function verifyCoverageInvariant(run) {
...
function runLocalAudit(inputs, options = {}) {
  const startedAt = options.createdAt ?? (/* @__PURE__ */ new Date()).toISOString();
...
function auditableBlocks(artifacts, sourceBlocks) {
  const artifactsById = new Map(artifacts.map((artifact) => [artifact.artifactId, artifact]));
  return sourceBlocks.filter((block) => block.normalizedText.length > 0 && block.blockType !== "heading").map((block) => ({ block, artifact: artifactsById.get(block.artifactId) })).filter((entry) => Boolean(entry.artifact));
}
function findExactDuplicates(artifacts, sourceBlocks, createdAt) {
...
function findNormalizedDuplicates(artifacts, sourceBlocks, createdAt) {
...
function findNearDuplicates(artifacts, sourceBlocks, createdAt, threshold) {
...
function duplicateFindings(groups, ruleId, category, createdAt) {
  const findings = [];
...
// packages/core/dist/exporters.js
...
function readGitMetadata(cwd, targetRef) {
  const repository = safeGit(["rev-parse", "--show-toplevel"], cwd) ?? cwd;
  const gitCommit = targetRef ? safeGit(["rev-parse", targetRef], cwd) ?? targetRef : safeGit(["rev-parse", "HEAD"], cwd) ?? "WORKTREE";
...
async function writeConfig(auditRoot) {
...
function shouldIgnoreDirectory(name, fullPath, cwd) {
